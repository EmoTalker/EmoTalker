...
class SyncNetInstance(torch.nn.Module):
    ...
    def evaluate(self, opt, videofile):
        ...
        command = ("ffmpeg -r 30 -y -i %s -threads 1 -f image2 -r 25 %s" % (videofile,os.path.join(opt.tmp_dir,opt.reference,'%06d.jpg'))) 
        ...
        for fname in flist:
            ...
            frame = TVF.center_crop(frame, output_size=1024)
            frame = TVF.resize(frame, size=224)
            # frame = TVF.resize(frame, output_size=256)
            # frame = TVF.center_crop(frame, output_size=224)
