...
class QuantizeEMAReset(nn.Module):
    ...
    def reset_codebook(self):
        ...
        self.register_buffer('codebook', torch.zeros(self.nb_code, self.code_dim).to(self.args.device))
...
class QuantizerFixCodebook(nn.Module):
    def __init__(self, n_e, e_dim, codebook):
        super(QuantizerFixCodebook, self).__init__()

        self.e_dim = e_dim
        self.n_e = n_e

        self.embedding = codebook
    ...
    def quantize(self, z):
        ...
        d = torch.sum(z ** 2, dim=1, keepdim=True) + \
            torch.sum(self.embedding ** 2, dim=1) - 2 * \
            torch.matmul(z, self.embedding.t())
        # B x 1
        min_encoding_indices = torch.argmin(d, dim=1)
        return min_encoding_indices
